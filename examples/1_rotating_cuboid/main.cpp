 ...
 * 
 *
 * ASSUMPTION: near plane at distance 1 (thus FOV = 90 degr)
 * -> if this weren't true, then it wouldn't be as easy as dividing by z.
 */
...
int main(void){
    ...
    */ 
    ...
    Vertex v1 = {.x =  base, .y =  base, .z =  - offset};
    Vertex v2 = {.x = -base, .y =  base, .z =  - offset};
    Vertex v3 = {.x = -base, .y = -base, .z =  - offset};
    Vertex v4 = {.x =  base, .y = -base, .z =  - offset};
    Vertex v5 = {.x =  base, .y =  base, .z =    offset};
    Vertex v6 = {.x = -base, .y =  base, .z =    offset};
    Vertex v7 = {.x = -base, .y = -base, .z =    offset};
    Vertex v8 = {.x =  base, .y = -base, .z =    offset};
    ...
    while(running) {
        ...
        */
	    ...
	    vts.at(i) = rot(vts.at(i), 1, X_AXIS); // rotation relative to camera (vts coordinates are relative to the camera, cam at origin)
            ...
            Vertex rotPoint = screen(project({.x = -x_off, .y = -y_off, .z = -z_off}), W, H);
        ...
        */
        ...
        for(int i =0 ; i < vts.size() ; i++){
           vts_cp.at(i) = project(vts_cp.at(i));
